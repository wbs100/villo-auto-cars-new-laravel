...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        View::composer('*', function ($view) {
            ...
            $minPrice = (float) Cache::remember('vehicles.min_price', 300, function () {
                return (float) (Vehicle::min('price') ?? 0);
            });
            $maxPrice = (float) Cache::remember('vehicles.max_price', 300, function () {
                return (float) (Vehicle::max('price') ?? 0);
            });
            ...
            $vehicleCountsByYear = Cache::remember('vehicle.counts.by.year', 120, function () {
                return Vehicle::groupBy('manufactured_year')
                    ...
                    ->mapWithKeys(function ($count, $k) {
                        return [trim((string)$k) => $count];
                    });
            });
            ...
            $vehicleCountsByCondition = Cache::remember('vehicle.counts.by.condition', 120, function () {
                ...
                return Vehicle::groupBy(DB::raw('LOWER(`condition`)'))
                    ...
                    ->mapWithKeys(function ($count, $k) {
                        return [trim(strtolower((string)$k)) => $count];
                    });
            });
            ...
            $years = Year::all()->map(function ($year) use ($vehicleCountsByYear) {
                ...
                if (is_null($count) && is_numeric($k)) {
                    $count = $vehicleCountsByYear->get((int)$k, 0);
                }
